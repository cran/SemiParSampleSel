ghssF2g <- function(params, dat, dat1, dat2, weights=weights, X1.d2, X2.d2, params.f, sp=NULL, qu.mag=NULL){
...
# Settings:
  ...
  settings <- cbind(F1,F2,f2,u,z,ph,b,B,h,ye,Gk,G2k,dF2k,df2kbyf2,d2F2k)
...
# (Minus) Hessian:
  ...
  H <- rbind( 
              cbind( sum(d2l.k.st.k.st), sum(d2l.k.st.teta) ) ,
              cbind( sum(d2l.k.st.teta), sum(d2l.teta.teta) )
            ) 
