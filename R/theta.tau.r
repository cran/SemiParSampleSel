theta.tau <- function( BivD, theta.star ) {
  ...
  if(BivD %in% c("N") )           { theta <- tanh(theta.star); 	   theta1 <- mean(theta);      KendTau <- tau(normalCopula(theta1))  }
  ...
  if(BivD=="FGM")                 { theta <- tanh(theta.star);	   theta1 <- mean(theta);      KendTau <- tau(fgmCopula(theta1))     }
  if(BivD=="F")                   { theta <- theta.star;           theta1 <- mean(theta);      KendTau <- tau(frankCopula(theta1))   }
  if(BivD=="AMH")                 { theta <- tanh(theta.star);	   theta1 <- mean(theta);      KendTau <- tau(amhCopula(theta1))     }
