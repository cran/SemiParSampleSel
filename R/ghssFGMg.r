ghssFGMg <- function(params, dat, dat1, dat2, weights=weights, X1.d2, X2.d2, sp=NULL, l.sp1, l.sp2, gp1, gp2, fp, qu.mag=NULL){
...
# Settings:
  ...
  settings <- cbind(F1,F2,f2,u,ph,h,ye,Gk,G2k,dF2k,df2kbyf2,d2F2k)
...
# (Minus) Hessian:
  ...
  d2l.be1.k.st    <- weights*i1*2*teta*u^(-2)*dF2k*ph*k
  d2l.be1.teta.st <- -weights*i1*(1-2*F2)*u^(-2)*ph*(1-teta^2)

  d2l.be2.be2     <- weights*i1*i2*( f2*h*(i2*f2*h-k*(1-ye)) + k*exp(-eta2) )
  d2l.be2.k.st    <- weights*i1*( i2*f2*h*(-dF2k*h+df2kbyf2) + 1 - ye )*k
  d2l.be2.teta.st <- -weights*i1*i2*2*F1*f2*u^(-2)*(teta^2-1)

  d2l.k.st.k.st    <- -weights*i1*( dF2k*h + df2kbyf2 + h*k*(d2F2k-dF2k^2*h) + 1 - k*psiprim )*k
  d2l.k.st.teta.st <- -weights*i1*dF2k*2*F1*u^(-2)*(1-teta^2)*k

  d2l.teta.st.teta.st <- -weights*i1*(u-1)*u^(-1)*( (1-teta^2)*u^(-1) - 1 - teta^2 )*(1-teta^2)*teta^(-2)
